...
class Model:
    ...
    def runRas(self):
        ...
        rasProjectFilePath = self.config.rasProjectPath + "/" + self.config.rasProjectName + ".prj"

        if self.config.rasProjectName == "TICR_Baseline":
            import shutil
            shutil.copyfile(self.config.rasProjectPath + "DesignRuns/" + self.config.hmsProjectName + ".dss", self.config.rasProjectPath + self.config.hmsProjectName + ".dss")
