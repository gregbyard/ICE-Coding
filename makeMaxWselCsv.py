...
def filterStations(filePath, XSfile):
    ...
    with open(filePath + XSfile, 'rb') as inputFile: #"StonyCkXS_Zoe.csv", 'rb') as inputFile:  #
        stationList = []
        stationAddress = {}
...
def writeToCSV(writeData, diffData, filePath, versions, XSfile):
    ...
    with open(outFileName, 'wb') as output:
        ...
        for k in writeData.keys():
            stationList, stationAddress = filterStations(filePath, XSfile)
            if k in stationList:
...
def getPeak(versions, filePath, XSfile):
    ...
    writeToCSV(elevData, diffElevData, filePath, versions, XSfile)
...
def main():
    ...
    getPeak(config.versions, config.filePath, config.XSfile)
